Shader "Unlit/NewUnlitShader"
{
    Properties
    {
        ...
        _ScreenSizeX ("screen size X", Int) = 1920
        _ScreenSizeY ("screen size Y", Int) = 1080
    }
    SubShader
    {
        Tags { "RenderType"="Opaque" "UniversalRenderPipeline" = "true" "LightMode" = "UniversalForward" }
        LOD 100
        ...
        Pass
        {
           ...
           int _ScreenSizeX;
            ...
            #define _REQUIRE_LightBuffers 1
    ...
    // world-space
            ...
            float4 frag (v2f i) : SV_Target
            {
                ...
                //float2 BlockUV = PixelateUV(screenUV, 10, screenSize);
                //float2 uv = ceil((i.uv / pixelSize) * pixelSize);
                float uv = normalize(i.uv);
                float3 albedo = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, ceil(i.uv * _PixelSize)/_PixelSize).rgb;
...
float3 totalLight = mainLight.color * NdotL;
                ...
                for (int j = 0; j < addCount; j++)
                {
                    ...
                    float NdotL1 = dot(normal, L);
                    //float distance = length(add.position - i.positionWS);
                    //float att = saturate(1.0 - distance / (1 * 10));
                    //float3 addPos = (unity_4LightPosX0, unity_4LightPosY0, unity_4LightPosZ0)
                   float lightRange = 1.0 / sqrt(add.distanceAttenuation);
                    
                    
                    float3 diffuse = add.color.rgb * NdotL1;                
